...
import source_adzuna, source_jsearch
...
@app.get("/health")
def health() -> dict:
    """Uptime ping plus a source-credential report.

    Reports only whether each variable is present and how long it is --
    never the value itself, so this stays safe to open in a browser.
    """
    def seen(name: str) -> dict:
        raw = os.getenv(name)
        if raw is None:
            return {"set": False, "note": "variable not found"}
        stripped = raw.strip()
        return {
            "set": bool(stripped),
            "length": len(stripped),
            "whitespace_trimmed": raw != stripped,
        }
    ...
    return {
        ...
        "sources": {
            "adzuna": {
                "usable": source_adzuna.available(),
                "ADZUNA_APP_ID": seen("ADZUNA_APP_ID"),
                "ADZUNA_APP_KEY": seen("ADZUNA_APP_KEY"),
            },
            "jsearch": {
                "usable": source_jsearch.available(),
                "RAPIDAPI_KEY": seen("RAPIDAPI_KEY"),
            },
        },
        "env_keys_present": sorted(
            k for k in os.environ
            if k.startswith(("ADZUNA", "RAPIDAPI", "SMTP", "DIGEST", "CRON", "DB_"))
        ),
        **store.stats(),
    }
